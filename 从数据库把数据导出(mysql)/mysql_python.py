...
if __name__ =='__main__':
    ...
    for num in range(1, long_count):
        ...
        sql = 'select id,DT,byq_name,Oper_line_name,Baseline_line_name from error_table where id = %s ' % num
        # #执行sql语句
        cur.execute(sql)
        # #获取所有记录列表
        data = cur.fetchone()
    ...
    print(big_list)
    head = ['id','DT',"byq_name","Oper_line_name","Baseline_line_name"]
    csv_dict_write('/home/w/error_table.csv',head,big_list)
    print("数据导出完成～")