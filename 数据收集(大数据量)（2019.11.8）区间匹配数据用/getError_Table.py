...
def insertDB(content):
    ...
    cursor = connection.cursor()
    try:
        cursor.executemany("insert into error_table (DT,byq_name,Oper_line_name,Baseline_line_name,q1_line_id,q2_LINE_ID) values (%s,%s,%s,%s,%s,%s)", content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    ...
    try:
        ...
        for item_q1 in l1:
            ...
            for item_q2 in l2:
                ...
                q2_LINE_NAME = item_q2[1]  # b2数据 元组拼接
                q2_LINE_ID = item_q2[2]  # b2数据 元组拼接

                if q1_shebei_id == q2_BN and q1_line_id != q2_LINE_ID :
