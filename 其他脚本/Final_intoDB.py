...
def insertDB(content):
    ...
    cursor = connection.cursor()
    try:
        cursor.executemany("insert into Final_inDB (DT,q1_line_id,NB_LINE_NAME,q1_stopC_time,NB_line_stopTime,NB_line_stopTime_Plus5,twoTime_cut) values (%s,%s,%s,%s,%s,%s,%s)", content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
